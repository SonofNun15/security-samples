...
export default function buildRouter(host: string, port: number) {
  ...
  router.post('/money', (req, res) => {
    ...
    // Guard against CSRF, but referer can be spoofed
    const referer = req.headers.referer || ''
    if (!referer.includes(`http://${host}:${port}`)) {
        sendLog(`REQUEST made from wrong origin: '${referer}'!`)
        res.render('csrf/login', { message: 'Bad origin, possibly CSRF' })
        return
    }
